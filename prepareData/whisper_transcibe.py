...
def _patch_hf_hub_use_auth_token() -> None:
    """Map deprecated use_auth_token for pyannote.audio with huggingface_hub >= 1.0."""
    import huggingface_hub

    if getattr(huggingface_hub.hf_hub_download, "_pyannote_compat", False):
        return
...
def _diarize_worker(audio_file: str, config_path: str, result_queue) -> None:
    ...
    # config.yaml model paths are relative to prepareData/
...
def transcribe_video_to_json(
    ...
    *,
    diarization_config: str | Path | None = None,
    skip_if_exists: bool = True,
) -> Path:
    """Extract audio, transcribe with diarization, and save transcript JSON."""
    video_path = Path(video_path)
    output_path = Path(output_path)
    output_path.parent.mkdir(parents=True, exist_ok=True)

    if skip_if_exists and output_path.exists():
        print(f"  [SKIP] transcript already exists: {output_path.name}")
        return output_path

    audio_path = extract_audio_mp3(video_path)
    try:
        segments, info = transcribe_audio(audio_path, loaded_model, diarization_config)
        dump_transcript_json(segments, info, output_path)
        print(f"  [OK]   transcript saved: {output_path.name}")
    finally:
...
if __name__ == "__main__":
    model_path = Path(__file__).parent.parent / "models" / "whisper_models" / "faster-whisper-medium"
    ...
    video_path = Path(__file__).parent.parent / "videos" / "380308012026RU1.mp4"
