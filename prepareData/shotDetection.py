...
def weighted_adaptive_threshold_shot_detection(
    ...
    output_file = os.path.join(output_dir, f"{video_stem}_weighted_adaptive_prediction.json")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run shot detection on a folder of videos.")
    parser.add_argument(
        "--input-dir",
        type=Path,
        default=Path(__file__).parent.parent / "data" / "videos",
        help="Folder containing input video files (default: data/videos/)",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path(__file__).parent.parent / "data" / "detected_shots",
        help="Folder where detected-shot JSON files are saved (default: data/detected_shots/)",
    )
    args = parser.parse_args()

    videos = (
        glob.glob(str(args.input_dir / "*.mp4"))
        + glob.glob(str(args.input_dir / "*.MP4"))
    )
